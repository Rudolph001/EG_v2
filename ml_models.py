...
class AdvancedAnalytics:
    ...
    def analyze_correlations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            dept_outcome = pd.crosstab(df['department'], df['final_outcome'])
            correlations['department_outcome'] = dept_outcome.to_dict()
            
            # Sender patterns
            sender_stats = df.groupby('sender').agg({
                'final_outcome': ['count', lambda x: (x == 'Escalated').sum()]
            }).round(3)
            high_risk_senders = sender_stats[
                sender_stats[('final_outcome', '<lambda>')] > 0
            ].head(10).to_dict()
            correlations['high_risk_senders'] = high_risk_senders
            
            # Feature correlations
            numeric_cols = ['has_attachments', 'is_leaver', 'external_sender']
            if len(numeric_cols) > 1:
                feature_corr = df[numeric_cols].corr().to_dict()
                correlations['feature_correlations'] = feature_corr
    ...
    def detect_anomalies(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            query = """
                SELECT id, sender, subject, department, attachments, final_outcome,
                       _time
                FROM emails 
                WHERE _time >= datetime('now', '-30 days')
                ORDER BY _time DESC
            """
            ...
            # Unusual sender frequency
            sender_counts = df['sender'].value_counts()
            unusual_senders = sender_counts[sender_counts > sender_counts.quantile(0.95)]
            for sender, count in unusual_senders.items():
                anomalies.append({
                    'type': 'high_frequency_sender',
                    'sender': sender,
                    'count': int(count),
                    'threshold': float(sender_counts.quantile(0.95))
                })
            
            # Unusual department activity
            dept_counts = df['department'].value_counts()
            if len(dept_counts) > 1:
                unusual_depts = dept_counts[dept_counts > dept_counts.quantile(0.9)]
                for dept, count in unusual_depts.items():
                    anomalies.append({
            ...
            unusual_hours = hourly_counts[
                (hourly_counts > hourly_counts.quantile(0.9)) |
                (hourly_counts < hourly_counts.quantile(0.1))
            ]
            for hour, count in unusual_hours.items():
                anomalies.append({
